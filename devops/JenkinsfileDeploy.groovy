            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Final Test 403 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Final Test 403 failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Final Test 403 deploying"
                ...
                slackSend color: "good",
                        message: "Final Test 403 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Final Test 403 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Final Test 403 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Final Test 403 failed to shut down properly"
            throw err
        }
    }
}
